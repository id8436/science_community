    ...
    if request.method == 'POST':  # 포스트로 요청이 들어온다면... 글을 올리는 기능.
        ...
        for i, tag in enumerate(box):
            if not tag:
                ...
                tag = tag.strip()  # 문자열 양쪽에 빈칸이 있을 때 이를 제거한다.
                tag_, created = Subjects.objects.get_or_create(base_exam=board, name=tag, sj_code=sj_code[i])  # 과목 생성.
        return redirect('boards:board_detail', board_id=board_id)  # 작성이 끝나면 작성한 글로 보낸다.
    else:  # 포스트 요청이 아니라면.. form으로 넘겨 내용을 작성하게 한다.
        form = PostingForm()
        subjects = Subjects.objects.filter(base_exam=board)  # 해당 보드에 해당하는 것을 가져온다.
...
def subject_register(request, board_id):
    ...
    if request.method == 'POST':  # 포스트로 요청이 들어온다면... 글을 올리는 기능.
        ...
        if form.is_valid():
            ...
            profile.save()
            subjects = Subjects.objects.filter(base_exam=board)
...
def subject_upload_excel_form(request, board_id):
    if request.method == "POST":
        ...
        for i in range(len(subject_info)-2):
            ...
            subject, created = Subjects.objects.get_or_create(base_exam=board, name=subject_name)
